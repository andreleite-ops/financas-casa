...
def para_centavos(valor) -> int:
    ...
    if "," in txt and "." in txt:
        ...
        # virgula com 3 digitos a direita e milhar (1,234), senao e decimal
        txt = txt.replace(",", "") if re.search(r",\d{3}$", txt) else txt.replace(",", ".")
    elif re.search(r"\.\d{3}$", txt) and txt.count(".") >= 1 and len(txt.split(".")[-1]) == 3:
        # 1.234 sem centavos -> milhar
        txt = txt.replace(".", "")
